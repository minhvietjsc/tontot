                ...
                <div class="category-list">
                    ...
                    <div class="listing-filter">
                        <div class="pull-left col-xs-6">
                            <div class="breadcrumb-list"><a href="javascript:void(0)" class="current"> <span>Tất cả quảng cáo của người dùng</span></a>
                            </div>
                        </div>
                        ...
                        @foreach($search_fields as $item)
                            <div class="col-md-3">
                                ...
                                <select name="custom_search[{{ strtolower( str_replace(' ', '_', $item->name) ) }}]" form="search_form" onchange="$('#search_form').submit()" class="form-control">
                                    <option value="">Tất cả {{ucfirst($item->name)}} </option>
                                    ...
                                    @foreach($result as $v)
                                        <div class="item-list make-list" style="">
                                            ...
                                            <div class="col-sm-8 add-desc-box col-md-7">
                                                ...
                                                <h5 class="add-title"><a href="{{url('tin/'.urlencode(str_slug(str_replace(' - ', '', $v->title.'-'.$v->id), '-')))}}">{{ ucfirst($v->title) }} </a></h5>
                                                <span class="info-row">
                                                    ...
                                                    - <span data-toggle="tooltip" data-placement="top" title="" data-original-title="{{ floor($v->visit/2) }} lượt xem quảng cáo "><i class="fa fa-eye" aria-hidden="true"> {{ floor($v->visit/2) }}</i></span>
                                                    - <span data-toggle="tooltip" data-placement="top" title="" data-original-title="{{ floor($v->message) }} tin nhắn "><i class="fa fa-envelope-o" aria-hidden="true"> {{ floor($v->message) }}</i></span>
                                                </span>
                                                </div>
                                            ...
                                            <div class="col-md-3 text-right  price-box">
                                                ...
                                                @if($v->user->is_verified==2)
                                                    <a class="btn bg-white  btn-sm btn-auths" > <img src="{{ asset('assets/img/vfy.png') }}" title="Verified user" alt="" height="22px"> <span>Người dùng đã xác minh</span> </a>
                                                @endif
            ...
            <div class="col-sm-3  page-sidebar-right">
                <aside>
                    <div class="panel sidebar-panel panel-contact-seller">
                        <div class="panel-heading">Liên hệ đại lý</div>
                                    ...
                                    <div class="company-logo-thumb col-md-9 row">
                                        <div class="m-l-5">
                                            ...
                                            @if($result[0]->user->mobile_verify == 1)
                                                <div class="btn btn-xs m-t-5 btn-success">Điện thoại đã xác minh </div>
                                            @endif
                                    ...
                                    <div class="text-leftt clearfix">
                                        ...
                                        @if($result[0]->user->type == 'c')
                                            @if($result[0]->user->telephone)
                                                <p>Điện thoại: <strong>{{ ucfirst($result[0]->user->telephone) }}</strong></p>
                                            @endif
                                            @if($result[0]->user->fax)
                                                <p>FAx: <strong>{{ ucfirst($result[0]->user->fax) }}</strong></p>
                                            @endif
                                            @if($result[0]->user->address)
                                                <p>Vị trí: <strong>{{ ucfirst($result[0]->user->address) }}</strong></p>
                                            @endif
                                        @endif
                        ...
                        <div class="panel-content">
                            <div class="panel-body text-left">
                                <ul class="list-check">
                                    ...
                                    <li> Kiểm tra các mặt hàng trước khi mua</li>
                                    <li> Chỉ trả tiền sau khi nhận hàng</li>
                                </ul>
    ...
    <script>
        ...
        $('.chathead').on('click', function () {
            ...
            if($('#chat .panel-body').hasClass('hidden')){
                if(hide_chat != 'hide') {
                    $('#chat .chat').find('center').html('<center> Chưa có cuộc trò chuyện nào! <br> Bắt đầu trò chuyện với <strong>' + $.cookie('chatUser') + '</strong> </center>');
                }
        ...
        // end chat function
        var hint = '{{$rating->total}} phiếu bầu, trung bình {{ floor($rating->avg) }} trên 5';
        (function ($) {
            $(function () {
