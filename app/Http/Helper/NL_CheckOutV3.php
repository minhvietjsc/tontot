...
use GuzzleHttp\Client;
  ...
  class NL_CheckOutV3
  {
									...
									$buyer_address,$array_items,$bank_code) 
		{
			...
			if(count($array_items)>0){
			 	foreach($array_items as $array_item){
					$params = array_merge($params, $array_item);
				}
			}

			//die($post_field);
			$params['query'] = $params;
			$nl_result=$this->CheckoutCall($params);
			return $nl_result;
    	}
    ...
    function QRCodeCheckout($order_code, $total_amount, $bank_code, $payment_type, $order_description, $tax_amount, $fee_shipping, $discount_amount, $return_url, $cancel_url, $buyer_fullname, $buyer_email, $buyer_mobile, $buyer_address, $array_items) {
        $params = array(
            'cur_code' => $this->cur_code,
            'function' => 'SetExpressCheckout',
            'version' => $this->version,
            ...
            'order_code' => $order_code, //Mã hóa đơn do website bán hàng sinh ra
            'total_amount' => $total_amount, //Tổng số tiền của hóa đơn						
            'payment_method' => 'QRCODE', //Phương thức thanh toán
			'bank_code' => $bank_code,
            'payment_type' => $payment_type, //Kiểu giao dịch: 1 - Ngay; 2 - Tạm giữ; Nếu không truyền hoặc bằng rỗng thì lấy theo chính sách của NganLuong.vn
            'order_description' => $order_description, //Mô tả đơn hàng
            'tax_amount' => $tax_amount, //Tổng số tiền thuế
	...
	function CheckoutCall($params){		
				
		$client = new Client(['base_uri' => $this->url_api]);
		...
		if ($statusCode == 200){	
	        ...
	        $nl_result = simplexml_load_string($body);
